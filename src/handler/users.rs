use axum::{extract::{Query, State}, middleware, response::IntoResponse, routing::{get, put, post, delete}, Extension, Json, Router};
use validator::Validate;
use axum_extra::extract::cookie::Cookie;
use axum::http::{header, HeaderMap, StatusCode};

use crate::{db::UserExt, dtos::{DoubleCheckDto, FilterUserDto, NameUpdateDto, RequestQueryDto, Response, RoleUpdateDto, UserData, UserListResponseDto, UserPasswordUpdateDto, UserResponseDto}, error::{ErrorMessage, HttpError}, middleware::{role_check, JWTAuthMiddleware}, models::UserRole, utils::password, AppState};
...
pub async fn delete_me(
    ...
    if passwords_match {
        // Delete user
        app_state.db_client.delete_user(user.id).await
            .map_err(|e| {
